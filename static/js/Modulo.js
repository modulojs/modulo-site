// Copyright 2025 MichaelB | https://modulojs.org | Modulo v0.1.0wip | LGPLv3
...
window.Modulo = class Modulo {
    constructor() {
        ...
        this.build = { }; // Records all that is in current build
        this.bundles = { script: [], style: [], link: [], meta: [], modscript: [], modstyle: [] };
        this.registry = { cparts: { }, coreDefs: { }, utils: { }, core: { },
                           engines: { }, commands: { }, templateFilters: { },
                           templateTags: { }, processors: { }, elements: { },
                           bundle: { }, modules: { } };
        //this.tools = { }; // TODO Have this get populated with injected versions after full config
    ...
    register(type, cls, defaults = undefined) { // TODO Mostly refactor away, along with core types, and instead be built when "load"?
...
modulo.register('processor', function mainRequire (modulo, conf, value) {
    modulo.config.modulo.build.mainModules.push(value); // TODO: Get helper to get top parent (e.g. Modulo), attach to definitions there
...
modulo.config.artifact = {
    ...
    BuildCommandFinalizers: [ 'Remove', 'SaveToStore' ],
    DefinedAs: 'name',
...
modulo.register('coreDef', class Artifact {
    ...
    static LoadElems (modulo, def, value) {
   ...
   static SaveTo (modulo, def, value, doc = null) { // Build processor
        ...
        //const render = s => new modulo.registry.cparts.Template(s, {}, modulo).render(ctx);
        const render = s => new modulo.registry.cparts.Template(s).render(ctx);
        const source = def.Content.replace(/[/g, '<').replace(/]/g, '>'); // substitute brackets
        const text = render(def.Content); // Run artifact and save data
        ...
        modulo.stores[value].setItem(def.path, (def.prefix || '') + text); // Save
    }
});



//             download="{{ path }}">&darr;</a></d><d>{{ text|length|divide:1024|round:1 }} kb</d></r>{%
    ...
    buildCallback() {
        ...
        for (const elem of this.element.querySelectorAll('*')) {
            for (const name of elem.getAttributeNames()) {
                if (/[\[\]\.@:]/.test(name)) { // TODO: make configurable, along with general syntax
                    elem.removeAttribute(name);
                }
            }
        }
        /*        for (const patch of this._mountPatchset || []) { // Look at every patch used when mounting
            if (patch[2] && patch[2].rawName) { // If a directive...
                patch[2].el.removeAttribute(patch[2].rawName); // Delete the attribute
            }
        }        */
    }
...
modulo.register('cpart', class Template {
    ...
    constructor(text, options = null) {
        if (typeof text === 'string') { // Using "new" (direct JS interface)
            //(modulo || window.modulo).instance(options || { }, null, this); // Setup object
...
modulo.register('processor', function contentLIST (modulo, def, value) {
    // TODO: Remove in favor of robust contentCSV with quoting
    const parse = s => s.trim().split('\n').filter(v => v.trim());
    def.Code = 'return ' + JSON.stringify(parse(def.Content || ''));
});
...
modulo.register('processor', function contentMD (modulo, def, value) {
    // TODO: remove, or combine with CSV in a simpler way without continuation? Or add general "extractor"?
    const chunks = str.split(/(<scr.pt type=.?md.?>---\n|^\s*---\n|\n---\n)/gi);
    const lines = chunks.length > 1 ? chunks[1].split(/[\n\r]/g) : [ ];
    let key = null; // Used to remember multi-line keys
    for (const line of lines) { // Loop through lines parsing meta
        if (key && /^[ \t]/.match(line)) { // Continuation
            def[key] += line.trim();
...
modulo.register('coreDef', class ContentList { // Generic static-data type
    static Register (modulo, def, value) { // Register dev commands
        for (const cmd of value.split(/\s+/g)) {
            modulo.registry.commands[cmd + '_' + def.DefinitionName] = (modulo) => {
                window._modSubFS = { FS: modulo.stores.FS, isSub: true, _cache };
                window._modSubFS._cache = modulo.fetchQueue.data; // Expose FS, cache
                for (const file of def.data.files) { // Loop through files to run
                    modulo.stores.PROC.setItem(file, cmd); // Set on PROC to run
                }
                modulo.cmdCallback(); // Setup dashboard & expose FS to iframes
            };
        }
    }
    ...
    DataType: 'LIST', // Default behavior is use "LIST" format (no auto)
    RequireData: 'DefinitionName',
    DefLoaders: [ 'DefTarget', 'DefinedAs', 'DataType', 'Src', 'commands|Register' ],
    DefBuilders: [ 'ContentLIST', 'ContentJSON', 'ContentJS', 'Code', 'RequireData', 'Load' ],
});

/*
//const fs = { FS: makeStoreFS(modulo), SH: makeStoreFS(modulo), PROC: makeStoreFS(modulo)};
// fs = Object.fromEntries((modulo.config.modulo.fs || [ 'OUT', 'FS', 'SH', 'PROC' ]).map(name => [name, makeStoreFS(modulo)]);
const { makeStoreFS, makeStore } = modulo.registry.utils;
const fs = { FS: makeStoreFS(modulo), SH: makeStore(modulo, { log: [] }) };
Object.assign(modulo.stores, fs, subFS || undefined);
//<p><a href="{{ m|get:'config.pathName' }}">&larr; {{ m|get:'config.pathName' }}</a></p>
*/
//modulo.stores.SH.data.log = def.files.map(f => [ f, cmd ]); // TODO change to PROC
// modulo.stores.PROC.setItem(file, cmd);
/* static ListFormat (modulo, def, value) { // Runs first to queue up ctx
    def.data = { files: (value || '').trim().split(/\s*[\n\r]\s* ?/g) };
    def.data.meta = Object.fromEntries(def.data.files.map(k => [k, { }]));
}
for (const file of def.data.files) {
    const proc = modulo.registry.processors['content' + value || file.split('.')[1]];
    modulo.fetchQueue.fetch(file).then(str => parse(modulo, meta[file], str));
} */
...
modulo.config.script = {
    lifecycle: null,
    Reserved: { // All reserved words in JavaScript (as obj for fast look-up)
        'break': 1, 'case': 1, 'catch': 1, 'class': 1, 'const': 1, 'continue': 1,
        'debugger': 1, 'default': 1, 'delete': 1, 'do': 1, 'else': 1,
        'enum': 1, 'export': 1, 'extends': 1, 'finally': 1, 'for': 1, 'if': 1,
        'implements': 1, 'import': 1, 'in': 1, 'instanceof': 1, 'interface': 1,
        'new': 1, 'null': 1, 'package': 1, 'private': 1, 'protected': 1,
        'public': 1, 'return': 1, 'static': 1, 'super': 1, 'switch': 1,
        'throw': 1, 'try': 1, 'typeof': 1, 'var': 1, 'let': 1, 'void ': 1,
        'while': 1, 'with': 1, 'await': 1, 'async': 1, 'true': 1, 'false': 1,
    },
...
modulo.register('cpart', class Script {
    static AutoExport (modulo, def, value) {
        ...
        def.exportNames = def.exportNames || getAutoExportNames(value, def.Reserved);
...
modulo.register('core', class Reconciler {
    ...
    parseDirectives(rawName, directiveShortcuts) { //, foundDirectives) {
        // TODO: Rewrite for . syntax, only looking for registered ones
...
modulo.register('util', function getAutoExportNames(text, banned = {}) {
    return (text.match(/(function|class)\s+(\w+)/g) || [])
        .map(s => s.match(/(function|class)\s+(\w+)/)[2])
        .filter(s => s && !(s in banned));
});
...
modulo.register('util', function makeStoreFS(modulo) { // TODO: Refactor state into core def!
    ...
    return Object.assign(store, { types: {} }, {
        ...
        setItem: (key, val) => {
            ...
            //store.data.log.push([ key, new Date().getTimestamp() ]); // TODO: Merge with makeStore, and add this as opt
            store.propagate('fdata', store.data.fdata);
        },
    });
});

modulo.config._dev = { dashboard: '<devlib-Dashboard>' };
modulo.config._dev.library = {
    artifact: `<Artifact name="css" -bundle="link,modstyle,style">
            {% for id in def.ids %}{{ def.data|get:id|safe }}{% endfor %}
        </Artifact>
        <Artifact name="js" -bundle="script,modscript" -collect="script[src$='mdu.js'],script[src$='/Modulo.js']">
            ...
            {% for name in config.modulo.build.mainModules %}
                modulo.registry.modules.{{ name }}.call(modulo, window);
            {% endfor %}
        </Artifact>
    ...
    component: `<Component namespace="devlib" name="Dashboard">
        <Template>
            <h1><a href="?argv={{ m.argv }}">&#x27F3; {{ m.argv|join:' ' }}</a></h1>
            {% if proc.log|length %}<aside>{% for argv in proc.log %}<iframe
                src="{{ argv|get:0 }}?argv={{ argv|get:1 }}"></iframe>{% endfor %}</aside>{% endif %}
            <pre>{% for argv in proc.log %}{{ argv|join:' \t' }}<br />{% endfor %}</pre>
            {% if not m.stores.isSub %}<pre>{% for path, text in build.fdata %}
                <a download="{{ path }}" href="data:text/plain;charset=utf-8,{{ text|urlencode:true|safe }}"
                >{{ path }}</a> ({{ text|length }}){% endfor %}{% endif %}
        </Template>
        <Script -name="m">function prepareCallback() { return modulo; }<` + `/Script>
...
modulo.register('util', function setupDevLib(modulo) {
    ...
    const subFS = window.self !== window.top ? window.parent._modSubFS : null;
    for (const fs of modulo.config._dev.fs || [ 'BUILD', 'CACHE', 'PROC' ]) {
        modulo.stores[fs] = subFS ? subFS[fs] : modulo.registry.utils.makeStoreFS(modulo);
    }
    modulo.fetchQueue.data = modulo.stores._cache || modulo.fetchQueue.data;
    for (const [ type, code ] of Object.entries(modulo.config._dev.library)) {
        modulo.loadString(code.replace(/^\s+/gm, ''), '_' + type);
    }
});

modulo.register('util', function getCommand(modulo) {
      const cmdName = modulo.argv.length > 0 ? modulo.argv[0] : '_default';
      modulo.cmdCallback = statusCode => {
          if (cmdName !== '_default') { // Don't modify DOM, in dev mode
              window.document.body.innerHTML = modulo.config._dev.dashboard;
          }
          modulo.cmdStatus = statusCode; // Indicate it has completed
      };
...
modulo.register('command', function _default (modulo) { // Show menu
    const href = 'window.location.href += ';
    const font = 'font-size: 28px; padding:0 8px 0 8px; border:2px solid #000;';
    const cmdNames = Object.keys(modulo.registry.commands).filter(cmd => !cmd.startsWith('_'));
    const commandGetters = cmdNames.map(cmd => ('get ' + cmd + ' () {' + href + '"?argv=' + cmd + '";}'));
    const clsCode = 'class {' + commandGetters.join('\n') + '}';
    modulo.cmdCallback(); // Should be a no-op
    Function(`console.log('%c%', '${ font }', "${ cmdNames.join(', ') }", new (${ clsCode }))`)();
});
...
if (typeof window.document !== 'undefined' && !window.moduloBuild) { // Browser
    ...
    window.document.addEventListener('DOMContentLoaded', () => {
        modulo.loadFromDOM(window.document.body, null, true); // Deferred load
